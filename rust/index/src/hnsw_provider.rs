...
const IS_LOAD_HNSW_FROM_MEMORY_ENABLED: bool = cfg!(feature = "load_hnsw_from_memory");
...
impl HnswIndexProvider {
    ...
    pub async fn fork(
        ...
        match self.get(&new_id, cache_key).await {
            ...
            None => {
                let index_raw = if IS_LOAD_HNSW_FROM_MEMORY_ENABLED {
    ...
    async fn open_hnsw_with_disk(
        ...
        println!("Creating directory: {}", index_storage_path.display());
        self.create_dir_all(&index_storage_path)
            .await
            .map_err(|e| Box::new(HnswIndexProviderOpenError::FileError(*e)))?;

        println!(
            "Loading hnsw segment into directory: {}",
            index_storage_path.display()
        );
        ...
        match self.get(id, cache_key).await {
            ...
            None => {
                let index_raw = if IS_LOAD_HNSW_FROM_MEMORY_ENABLED {
                    ...
                    self.open_hnsw_with_disk(id, *id, prefix_path, &index_config, ef_search)
                        .await
                        .map_err(|e| Box::new(HnswIndexProviderOpenError::IndexLoadError(e)))?
                };
                let index = HnswIndexRef {
...
mod tests {
    ...
    async fn test_open() {
        ...
        // clear the cache.
        provider.cache.clear().await.expect(
            "Expected to clear

        // delete storage_dircache",
        );
